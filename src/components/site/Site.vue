  ...
  <div class="site">
    ...
    <div class="main">
      ...
      <ul class="addr-list block" v-if="sites.length>0">
        <li class="addr-item spline-bottom" v-for="(item,index) in sites" :key="item.id">
          <div class="addr-desc" @click="confirmSite(item)"
          :class="{selected:item.id==selectedSite.id}">
...
export default {
  ...
  methods: {
    ...
    // 点击选的地址后,将users数据库里的地址selectedSite更改 这个selectedSite是用户的收获地址
    confirmSite (site) {

      this.$store.dispatch('confirmSite',site)
...
.notice-icon{
  ...
  height: 8rem;
  border-radius: 4rem;
  background: url("img/empty_addrlist.png") center center no-repeat;
...
.addr-edit{
  ...
  top: 1.5rem;
  right: 0;
  background: url("img/addr-edit.png") center center no-repeat;
