...
export default {
  created () {
    // 获取城市列表数据
    this.$http.get('http://localhost:8081/citys')
      .then(data => {
  ...
  methods: {
    ...
    save () {
      ...
      if (userInfo.id && this.city !== '' && this.site !== '' && this.linkman !== '' 
      && this.phone !== '' && this.detailSite !== '' && location.lng) {
        let sites = {
        ...
        this.$store.dispatch('saveSite', sites)
         .then(res => {
           this.$msg('提示',res)
         })
         .then(res => {
...
.checked.theme-radio:before {
  background-image: url('../shoppingCart/img/checkbox-selectd.png');
